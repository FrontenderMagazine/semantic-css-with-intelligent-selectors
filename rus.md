...
раздавлены тяжестью колосcального строения. В дизайне **на первом месте всегда 
...
Это что касается архитекторов. А как насчёт фронт-енд архитекторов, хоть нас 
часто и не считают архитекторами всерьез? Должны ли мы следовать этому правилу 
или им можно пренебречь?

С появлением [объектно-ориентированого CSS][2] (OOCSS) стало очень модно 
«[разделять семантику представления и семантику документа][3]». Используя 
...
В этой статье мы изучим альтернативный подход к описанию стиля веб-документов, 
...
разметку. Если у вас все в порядке с кодом, и дизайн у вас получится такой как 
нужно.
...
С изобретением таблиц стилей у нас появилась возможность физически разделять код 
разметки документа и код, отвечающий за его представление. Это помогает писать 
более грамотный и соответствующий стандартам код ровно настолько насколько 
...
абзацев), необходимость сохранения целостности представления вебсайта и его 
...
Неумные селекторы увеличивают количество времени на разработку вебсайта, потому 
...
них. Одним из недостатков такого расположения дел является то что CSS утрачивает 
маневренность, тоесть его нельзя применить для HTML-документа не прописав 
предварительно классы в соответствующих местах.
...
на умные селекторы. Тем не менее, иногда применение неумных селекторов доходит 
до абсурда и может привести к **дисбалансу** между структурой и презентацией 
...
спецификации HTML. Чтобы построить сложный умный селектор, можно с помощью 
комбинации **контекстного и функционального определения** провести различие 
между базовыми элементами. Некоторые элементы, например `<a>`, обладают 
...
которая соответствует документации BEM «[Определение][6]». 
...
к перегруженности документа описанной выше, уверен что вы уже используете их на 
...
ваш `abbr` может обозначать и Общество охраны памятников, и Организацию 
освобождения Палестины. 

Некоторые из этих атрибутов нужны чтобы улучшить семантичность документа, другие 
отвечают за корректное отображение и функционирование элемента. Если они 
...
различными агентами пользователя для улучшения пользовательского опыта и 
...
некоторые атрибуты были удалены. Представим что мы добавили перед текстом ссылки 
`[rel="prev"]` стрелку с помощью псевдоэлемента:
...
агентами пользователя. Теперь задумайтесь, ведь чтобы все эти ссылки выполняли 
разные функции, у них должны быть различия в определении. Тоесть, чтобы работать 
...
Маленькая нерешённая проблемка: что если наша ссылка не содержит ни один из 
...
производительности будут рады услышать что он не мог бы быть более простым. 

![a][Простой селектор (a)]

Поехидничали и хватит, могу вас уверить что наследование в каскаде работает для 
...
она соответствует первым двум критериям «теста на утку». Для всех пользователей, 
которые способны применить индуктивное мышление и уловить суть понятия «кнопка», 
она должна и крякать соответственно. Поскольку в наличии всего лишь ссылка, 
скринридеры распознают её как ссылку, следовательно ваш аллегорический небоскрёб 
не приспособлен для инвалидных колясок. Стремление избежать путаницы такого рода 
для пользователей вспомогательных технологий является не гонкой за семантической 
безупречностью, а нашей прямой обязанностью на общее благо.
...
селекторы атрибута и селектор `:not` (или же [псевдокласс отрицания][24]) чтобы 
определить проблемы в HTML. В отличии от других реализаций, в нашей уведомление 
об ошибке на экран будет выводиться с помощью псевдо-контента. Каждая ошибка 
...
> «Если элемент является ссылкой, у него должен быть атрибут `href`»
    ...
    a:not([href]):after {
       content: 'Это должна быть ссылка или кнопка? Она ни к чему не привязана!';
...
существование атрибута, т.е. совпадает ли `[href]` с элементом, у которого есть 
атрибут `href`. Эта проверка должна осуществляться только для гиперссылок, для 
этого используется префикс `a`. Правило можно прочитать так: «Для каждого 
элемента `a`, который не содержит атрибут `[href]`, должен быть добавлен 
псевдо-контент с сообщением об ошибке.»
...
потому что некоторые браузеры интерпретируют кнопки в таком контексте без чётко 
указанного типа как `type="submit"`. Мы должны быть полностью уверены что кнопке 
не будет присвоена функция отправки информации на сервер если она должна 
отвечать за какое-либо другое действие. 

### Правило 7

> «И ссылки, и кнопки должны содержать какой-либо контент или метку ARIA»
...
уследить? Нельзя полагаться на тестирование, потому что на этом этапе уже как 
правило поздно.
...
название][31]. Вы начнете тратить меньше времени на пользование пультом 
управления от телевизора и больше на его рассматривание и обдумывание. 
